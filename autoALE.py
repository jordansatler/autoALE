...
def run_ale_observe(tr):
    """run ALEobserve"""
    subprocess.call(["ALEobserve", tr, "burnin=1000"])

def run_ale(genetrees, ale, model):
    """run ALEml"""
    if ale == "undated":
        cmd = ["ALEml_undated" , "SpeciesTree.tre", genetrees + '.ale', "sample=1000"]
        if len(model) > 1:
            cmd.extend(model[1:])
    else:
        cmd = ["ALEml" , "SpeciesTree.tre", genetrees + '.ale', "sample=1000"]
        if len(model) > 1:
            cmd.extend(model[1:])
...
def main():
    if len(sys.argv) < 5:
        print "python autoALE.py figTrees figwaspTrees traits Dated/Undated model choice [y|n]"
        sys.exit()
